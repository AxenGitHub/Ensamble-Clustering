...
ConsensusMatrixAnalysis <- function(datasetName=NULL, chgCl=chgCl_global, minCl=minCl_global, numCl=numCl_global, numIter=numIter_global, std=std_global, sphere=sphere_global, tol=tol_global){
  #set the minimum number of clusters to the max number of clusters in order to execute only once the internal do-loop
  if(chgCl == FALSE){
    minCl = numCl
  }
